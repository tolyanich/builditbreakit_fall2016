...
func NewStore(adminPassword string) *Store {
	return &Store{
		users:     map[string]string{adminUsername: adminPassword, "anyone": ""},
		vars:      make(map[string]interface{}),
		permState: PermissionsState{defaultDelegator: "anyone", assertions: make(map[string][]Assertion)},
	}
}
...
func (ls *LocalStore) CreatePrincipal(username string, password string) error {
	...
	if ls.getDefaultDelegator() != "anyone" {
